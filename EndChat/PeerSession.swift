...
final class PeerSession: NSObject, ObservableObject {
    ...
    private var trustedContactNames: [String: String] = [:]
    ...
    private var pendingResources: [UUID: PendingResource] = [:]
    private var activeResourceTransfers = Set<UUID>()
    private var rejectedPeers = Set<MCPeerID>()
    ...
    deinit { retryTimer?.invalidate(); relayTask?.cancel() }
    ...
    func updateVerifiedContacts(_ contacts: [Contact]) {
        ...
        trustedContactNames = Dictionary(uniqueKeysWithValues: contacts.compactMap { contact in
            guard let id = contact.peerIdentifier, contact.verifiedPublicKey != nil else { return nil }
            return (id, contact.name)
        })
        for (id, key) in trustedPublicKeys where knownPublicKeys[id] == key {
            verifiedPeerIdentifiers.insert(id)
        }
        for (id, key) in trustedPublicKeys { knownPublicKeys[id] = key }
        persistKnownPublicKeys()
        for peer in discoveredPeers {
            guard let id = peerIdentifiers[peer], trustedPublicKeys[id] != nil else { continue }
            mergeLegacyThread(for: peer, into: id)
            if let index = threads.firstIndex(where: { $0.id == id }), let name = trustedContactNames[id] {
    ...
    func invite(_ peer: MCPeerID) {
        ...
        let context = PeerInviteContext(
            identifier: deviceIdentifier,
            publicKey: encryptionKey.publicKey.rawRepresentation
        )
        browser.invitePeer(peer, to: session, withContext: try? JSONEncoder().encode(context), timeout: 30)
    }
    ...
    func sendText(_ text: String, to conversationID: String) {
        ...
        connectToDiscoveredPeer(for: conversationID)
        retryPendingMessages()
    }

    private func connectToDiscoveredPeer(for conversationID: String) {
        guard let peer = discoveredPeers.first(where: {
            peerIdentifiers[$0] == conversationID || "peer:\($0.displayName)" == conversationID
        }) else { return }
        invite(peer)
    }

    private func mergeLegacyThread(for peer: MCPeerID, into identifier: String) {
        let legacyID = "peer:\(peer.displayName)"
        guard legacyID != identifier else { return }
        for index in messages.indices where messages[index].conversationID == legacyID {
            messages[index].conversationID = identifier
        }
        let transferIDs = pendingResources.compactMap { $0.value.conversationID == legacyID ? $0.key : nil }
        for id in transferIDs {
            pendingResources[id]?.conversationID = identifier
        }
        if let legacy = threads.first(where: { $0.id == legacyID }) {
            threads.removeAll { $0.id == legacyID }
            if let existing = threads.firstIndex(where: { $0.id == identifier }) {
                threads[existing].updatedAt = max(threads[existing].updatedAt, legacy.updatedAt)
            } else {
                threads.append(ChatThread(id: identifier, title: trustedContactNames[identifier] ?? peer.displayName, updatedAt: legacy.updatedAt))
            }
        }
    }
    ...
    func retryMessage(_ id: UUID) {
        guard let index = messages.firstIndex(where: { $0.id == id && $0.isMine }),
              let conversationID = messages[index].conversationID else { return }
        messages[index].deliveryState = .queued
        if messages[index].kind == .image || messages[index].kind == .file,
           let path = messages[index].localFilePath,
           let resourceName = messages[index].resourceName {
            pendingResources[id] = PendingResource(
                messageID: id,
    ...
    func giveUpMessage(_ id: UUID) {
        pendingResources.removeValue(forKey: id)
        activeResourceTransfers.remove(id)
    ...
    func sendImageData(_ data: Data, name: String = "Image.jpg", to conversationID: String) {
        ...
        do {
            ...
            messages.append(ChatMessage(
                sender: "EndChat",
                body: "Could not prepare image: \(error.localizedDescription)",
                isMine: false,
                kind: .system,
                conversationID: conversationID
            ))
    ...
    func sendFile(_ url: URL, to conversationID: String) throws {
        ...
        let outbox = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("Outbox", isDirectory: true)
        try FileManager.default.createDirectory(at: outbox, withIntermediateDirectories: true)
        let storedURL = outbox.appendingPathComponent("\(UUID().uuidString)-\(url.lastPathComponent)")
        if url.standardizedFileURL != storedURL.standardizedFileURL {
            try FileManager.default.copyItem(at: url, to: storedURL)
        }

        let messageID = UUID()
        let resourceName = messageID.uuidString + (url.pathExtension.isEmpty ? "" : ".\(url.pathExtension)")
        ...
        pendingResources[messageID] = PendingResource(
            messageID: messageID,
            conversationID: conversationID,
            url: storedURL,
            resourceName: resourceName
    ...
    private func sendWireMessage(from message: ChatMessage) -> Bool {
        ...
        if !sentOnLAN, message.kind == .text, let destination = message.conversationID, !lanOnly {
    ...
    private func retryPendingResources() {
        for transfer in pendingResources.values where !activeResourceTransfers.contains(transfer.messageID) {
            guard let message = messages.first(where: { $0.id == transfer.messageID }),
                  message.deliveryState == .sending || message.deliveryState == .delivered,
                  let peer = peers(for: transfer.conversationID).first else { continue }
            activeResourceTransfers.insert(transfer.messageID)
            transferStatus = "Sending \(message.fileName ?? "image")"
            session.sendResource(at: transfer.url, withName: transfer.resourceName, toPeer: peer) { [weak self] error in
                Task { @MainActor in
                    if let error {
                        self?.transferStatus = "Confirming delivery…"
                        try? await Task.sleep(for: .seconds(1.5))
                        guard let self else { return }
                        self.activeResourceTransfers.remove(transfer.messageID)
                        if self.messages.first(where: { $0.id == transfer.messageID })?.deliveryState == .delivered {
                            self.pendingResources.removeValue(forKey: transfer.messageID)
                            self.transferStatus = "Sent \(message.fileName ?? "image")"
                            return
                        }
                        if let index = self.messages.firstIndex(where: { $0.id == transfer.messageID }) {
                            self.messages[index].deliveryState = .failed
                        }
                        self.messages.append(ChatMessage(
                            sender: "EndChat",
                            body: "Image transfer failed: \(error.localizedDescription)",
                            isMine: false,
                            kind: .system,
    ...
    private func receiveSecurePacket(_ packet: SecurePacket, from peer: MCPeerID) {
        ...
        case .keyExchange:
            ...
            if let id = peerIdentifiers[peer], let trusted = trustedPublicKeys[id], trusted != rawKey {
                rejectedPeers.insert(peer)
                messages.append(.system("Blocked a connection whose encryption key did not match the verified contact."))
                return
            }
            rejectedPeers.remove(peer)
            ...
            retryPendingMessages()
        case .encrypted:
            guard !rejectedPeers.contains(peer) else { return }
    ...
    private func receive(_ wire: WireMessage, from peer: MCPeerID) {
        if let existing = messages.first(where: { $0.id == wire.id }) {
            if existing.kind == .text || existing.kind == .system || existing.localFilePath != nil {
        ...
        messages.append(
            ChatMessage(
                ...
                localFilePath: wire.resourceName.flatMap { receivedResourcePaths[$0] },
                conversationID: conversationID,
        ...
        touchThread(conversationID)
        if wire.kind == .text || wire.kind == .system || messages.last?.localFilePath != nil {
            sendAcknowledgement(for: wire.id, to: peer)
        }
    }
    ...
    private func receiveRelayPacket(_ packet: RelayPacket) {
        ...
        case .message:
            ...
            guard wire.kind == .text || wire.kind == .system else { return }
    ...
    private func loadMessages() {
        if let data = UserDefaults.standard.data(forKey: "messages"),
           let saved = try? JSONDecoder().decode([ChatMessage].self, from: data) {
            messages = saved.filter { !$0.body.hasPrefix("Could not load that photo:") }
        } else {
    ...
    nonisolated func session(_ session: MCSession, peer peerID: MCPeerID, didChange state: MCSessionState) {
        Task { @MainActor in
            ...
            case .notConnected:
                ...
                rejectedPeers.remove(peerID)
                if let conversationID = peerIdentifiers[peerID] {
                    for index in messages.indices where messages[index].isMine &&
                        messages[index].conversationID == conversationID &&
                        messages[index].deliveryState == .sending {
        ...
        Task { @MainActor in
            ...
            do {
                ...
                if let index = messages.lastIndex(where: {
                    !$0.isMine && $0.resourceName == resourceName && ($0.kind == .image || $0.kind == .file)
                }) {
                    messages[index].localFilePath = destination.path
                    sendAcknowledgement(for: messages[index].id, to: peerID)
                }
...
extension PeerSession: MCNearbyServiceAdvertiserDelegate {
    ...
    nonisolated func advertiser(
        ...
        Task { @MainActor in
            if let data = context,
               let invite = try? JSONDecoder().decode(PeerInviteContext.self, from: data) {
                peerIdentifiers[peerID] = invite.identifier
                mergeLegacyThread(for: peerID, into: invite.identifier)
                if trustedPublicKeys[invite.identifier] == invite.publicKey {
...
extension PeerSession: MCNearbyServiceBrowserDelegate {
    ...
    nonisolated func browser(_ browser: MCNearbyServiceBrowser, foundPeer peerID: MCPeerID, withDiscoveryInfo info: [String: String]?) {
        Task { @MainActor in
            ...
            if let identifier = info?["id"] {
                peerIdentifiers[peerID] = identifier
                mergeLegacyThread(for: peerID, into: identifier)
                if trustedPublicKeys[identifier] != nil {
                    invite(peerID)
                    return
                }
                if threads.contains(where: { $0.id == identifier }) {
                    invite(peerID)
                }
            }
        }
